...
export function ServiceAreasSettingsPanel({
  ...
  return (
    <div className="admin-settings-section service-area-settings-section" id="admin-service-areas">
      <fieldset disabled={saveStatus.loading || saveStatus.saving} style={{ border: 0, margin: 0, padding: 0, minWidth: 0 }}>
      ...
      {saveStatus.message && (
        <p className={`weekly-working-message ${saveStatus.type}`} role={saveStatus.type === "error" ? "alert" : "status"}>
          {saveStatus.message}
        </p>
      )}
      <div className="weekly-working-footer">
        <button type="button" className="admin-primary-action" onClick={onSave}
          disabled={!saveStatus.ready || !saveStatus.dirty || saveStatus.loading || saveStatus.saving}>
          {saveStatus.saving ? "Saving area settings..." : "Save area settings"}
        </button>
        {!saveStatus.ready && !saveStatus.loading && (
          <button type="button" className="admin-secondary-action" onClick={onRetry}>Retry loading areas</button>
        )}
      </div>
    </div>
  );
